...
interface UserSession {
  ...
  botRunning: boolean;
  tradingWallets: TradingWallet[];  // Current active trading wallets
  tradingWalletsHistory: TradingWallet[][]; // Array of arrays - keeps ALL past trading wallets
  createdAt: number;
...
function validateAndFixSession(session: UserSession): boolean {
  ...
  // Check if wallet exists but private key is missing
  if (session.walletKeypair && !session.userWalletPrivateKey) {
    console.log(`⚠️ User ${session.userId}: walletKeypair exists but userWalletPrivateKey is missing`);

    // Try to recover from walletKeypair
    try {
      ...
      // If recovery fails, clear the corrupted data
  ...
  // Check if depositAddress exists but wallet keys are missing
  if (session.depositAddress && !session.walletKeypair) {
    console.log(`⚠️ User ${session.userId}: depositAddress exists but walletKeypair is missing - wallet is LOST`);
    // Clear the orphaned address
    session.depositAddress = undefined;
    needsFix = true;
  }
...
// Save individual user session to their own file
function saveIndividualUserSession(userId: number, session: UserSession) {
  ...
  const exportData = {
    ...
    // Current active trading wallets
    ...
    // ALL trading wallets ever created (including old ones)
...
function saveSessions() {
  try {
    ...
    userSessions.forEach((session, userId) => {
      ...
      // Also save individual user file if they have a wallet
...
// Enhanced getUserSession with validation
function getUserSession(userId: number, chatId: number): UserSession {
  ...
  if (!session) {
    ...
    session = {
      ...
      botRunning: false,
      tradingWallets: [],
      tradingWalletsHistory: [], // Initialize history
      createdAt: Date.now(),
      tradingStats: {
        totalBuys: 0,
        totalSells: 0,
    ...
    // Initialize history if it doesn't exist (for old sessions)
    if (!session.tradingWalletsHistory) {
      session.tradingWalletsHistory = [];

      // If there are existing trading wallets, add them to history
      if (session.tradingWallets && session.tradingWallets.length > 0) {
        session.tradingWalletsHistory.push([...session.tradingWallets]);
        console.log(`Migrated ${session.tradingWallets.length} existing wallets to history for user ${userId}`);
      }
...
function validateAllSessions() {
  console.log('🔍 Validating all user sessions...');
  ...
  userSessions.forEach((session, userId) => {
    const wasFixed = validateAndFixSession(session);

    if (wasFixed) {
      fixedSessions++;
    }

    // Check if wallet is truly lost
    if (session.depositAddress && !session.walletKeypair && !session.userWalletPrivateKey) {
      lostWallets++;
      console.log(`❌ User ${userId}: Wallet ${session.depositAddress} is LOST (no private key)`);
    }
  ...
  console.log(`   - Valid wallets: ${validSessions}`);
  console.log(`   - Fixed sessions: ${fixedSessions}`);
  console.log(`   - Lost wallets: ${lostWallets}`);
}
...
// Handler to export session file
async function handleExportSession(userId: number, chatId: number, messageId?: number) {
  try {
    ...
    const message =
      ...
      `✅ All trading wallet keys (${session.tradingWallets.length} wallets)\n` +
      `✅ Token information\n` +
      `✅ Trading statistics\n` +
...
// Handler to show main wallet key
...
if (BOT_TOKEN) {
  ...
  function getWalletBenefits(count: number): string {
    switch (count) {
      case 1:
        return `• Single wallet operation\n• Minimal setup\n• Quick testing`;
      case 6:
  ...
  async function handleCreateWallet(userId: number, chatId: number, messageId?: number) {
    try {
      ...
      if (!session.walletKeypair) {
        ...
        // Set ALL fields at once to prevent partial save
        ...
        console.log(`   Private key saved: ${!!verifySession.userWalletPrivateKey}`);
        ...
        const message =
          ...
          `🔐 Your Private Key:\n` +
          `\`${privateKey}\`\n\n` +
          `⚠️ IMPORTANT: Your keys are auto-saved!\n` +
          ...
          `⚠️ This message will auto-delete in 120 seconds for security.`;
        ...
        // Wallet already exists - validate it
        validateAndFixSession(session);

        if (!session.userWalletPrivateKey) {
          // Still no private key after validation - wallet is lost
          const message =
            `⚠️ Wallet Recovery Issue Detected\n\n` +
            `Your wallet address exists but the private key was not saved properly.\n\n` +
            `💳 Address: \`${session.depositAddress}\`\n` +
            `❌ Private Key: NOT RECOVERABLE\n\n` +
            `⚠️ This wallet cannot be accessed anymore.\n` +
  ...
  function handleStopVolume(userId: number, chatId: number, messageId?: number) {
    try {
      ...
      // IMPORTANT: DON'T clear tradingWallets when stopping
      // They should only be moved to history when starting a NEW session
      // or explicitly withdrawn by the user
...
async function performWithdrawal(session: UserSession, withdrawAddress: string, chatId: number) {
  try {
    ...
    // Collect ALL trading wallets (current + historical)
    const allTradingWallets: TradingWallet[] = [];
    ...
    if (uniqueWallets.length > 0) {
      safeSendMessage(chatId, `⏳ Gathering SOL from ${uniqueWallets.length} total trading wallets (including old wallets)...`);
      ...
      for (let i = 0; i < uniqueWallets.length; i++) {
        try {
          ...
          if (balance === 0) {
            console.log(`Wallet ${i + 1}/${uniqueWallets.length} has 0 balance, skipping...`);
            continue;
          }
          ...
          if (transferAmount <= 5000) {
            console.log(`Wallet ${i + 1}/${uniqueWallets.length} has insufficient balance to transfer: ${balance / LAMPORTS_PER_SOL} SOL`);
            continue;
          }
          ...
          if (sig) {
            ...
            successfulGathers++;
            console.log(`✅ Gathered from wallet ${i + 1}/${uniqueWallets.length}: ${gatheredAmount.toFixed(6)} SOL`);
          }

          await sleep(1500);

        } catch (error: any) {
          console.error(`Failed to gather from wallet ${i + 1}/${uniqueWallets.length}:`, error);
          continue;
        }
      }
      ...
      if (successfulGathers > 0) {
        safeSendMessage(chatId,
          `✅ Gathered ${totalGathered.toFixed(6)} SOL from ${successfulGathers}/${uniqueWallets.length} wallets!\n` +
    ...
    // Continue with main wallet withdrawal
    ...
    if (txSig) {
      // CRITICAL FIX: Move current wallets to history, DON'T DELETE THEM
      if (session.tradingWallets && session.tradingWallets.length > 0) {
        if (!session.tradingWalletsHistory) {
          session.tradingWalletsHistory = [];
        }
        // Archive current wallets with withdrawal timestamp
        session.tradingWalletsHistory.push([...session.tradingWallets]);
        console.log(`🗂️ Archived ${session.tradingWallets.length} wallets after withdrawal for user ${session.userId}`);
      }

      // Clear ONLY the current trading wallets (they're now in history)
      session.tradingWallets = [];

      // DO NOT clear tradingWalletsHistory - this keeps ALL old wallet keys forever
      ...
      console.log(`Total wallets in history: ${session.tradingWalletsHistory?.length || 0} batches`);
    } else {
...
async function distributeSol(mainKp: Keypair, distributionNum: number, session: UserSession): Promise<{ kp: Keypair, address: string, privateKey: string }[]> {
  try {
    ...
    if (txSig) {
      ...
      // CRITICAL: Save old trading wallets to history BEFORE replacing
      if (session.tradingWallets && session.tradingWallets.length > 0) {
        if (!session.tradingWalletsHistory) {
          session.tradingWalletsHistory = [];
        }
        // Archive the old batch
        session.tradingWalletsHistory.push([...session.tradingWallets]);
      ...
      const message =
        ...
        `Note: Old wallet keys are safely archived`;
